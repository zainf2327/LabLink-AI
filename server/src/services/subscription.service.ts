...
import SubscriptionPlan, { ISubscriptionPlan } from '../models/SubscriptionPlan.model.js';
...
export const subscriptionService = {
  ...
  async createSubscriptionFromPlan(
    ...
    options?: { session?: mongoose.ClientSession; status?: 'active' | 'expired' | 'cancelled' }
  ...
  async initializeFamilyMembers(
    ...
    if (totalFamilyCount <= plan.maxFamilyMembers) {
      ...
      subscription.activeFamilyMemberIds = familyMembers.map((m) => m._id as mongoose.Types.ObjectId);
