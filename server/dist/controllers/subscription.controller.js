...
export const createSubscriptionIntent = asyncHandler(async (req, res) => {
    if (!req.user) {
    ...
    const intent = await stripeService.createPaymentIntent(amountInCents, 'usd', plan._id.toString(), `sub_${req.user.id}_${new Date().getTime()}` // idempotency key
    );
    const payment = new Payment({
        paymentFor: 'subscription',
        subscriptionPlanId: plan._id,
...
export const updateActiveFamilyMembers = asyncHandler(async (req, res) => {
    ...
    const currentActiveIds = req.subscription.activeFamilyMemberIds.map((id) => id.toString());
    const deactivatedMemberIds = currentActiveIds.filter((id) => !activeFamilyMemberIds.includes(id));
    if (deactivatedMemberIds.length > 0) {
        const hasUpcoming = await Booking.exists({
            forMemberId: { $in: deactivatedMemberIds.map((id) => new mongoose.Types.ObjectId(id)) },
